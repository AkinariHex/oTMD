...
async function updater() {
	const { apiKey, matchId, warmups, bestOf, reverse } = await fetch(`/settings`).then((res) => res.json())
	await updateTeams(apiKey, matchId)
	await matchDataLoop(apiKey, matchId, warmups, reverse, bestOf)
}

// on window loading and every 2 seconds calls the updater fn avoiding the use of websockets, but increases cpu workload
// refactor this later
...
async function updateTeams(apiKey, matchId) {
	const data = await fetch(`https://osu.ppy.sh/api/get_match?k=${apiKey}&mp=${matchId}`).then((res) => res.json())
	console.log(data)
	const { match } = data
	...
	countries.forEach(({ country, id }) => {
		if (team1Name.includes(country)) team1Img = `<img class="countryimg" src="https://osu.ppy.sh/images/flags/${id}.png"> <br />`
		if (team2Name.includes(country)) team2Img = `<img class="countryimg" src="https://osu.ppy.sh/images/flags/${id}.png"> <br />`
	})
...
async function matchData(apiKey, matchId, warmups, reverse, bestOf) {
	const { match, games } = await fetch(`https://osu.ppy.sh/api/get_match?k=${apiKey}&mp=${matchId}`).then((res) => res.json())
	const finished = !!match.end_time
	let team1Score = 0
	let team2Score = 0
	for (const { scores, team_type } of games.slice(warmups)) {
		if (team_type !== '2') throw new Error("Match isn't in Team VS mode")
		// ? We might not want reduce here as it can impact the performance
		const [team1, team2] = scores.reduce(
			(acc, { team, score }) => {
		...
		if (team1 > team2) team1Score += 1
		else if (team2 > team1) team2Score += 1
	}
	...
	if ((team1Score === maxScore || team2Score === maxScore || finished) && matchInterval) {
		if (team1Score > team2Score) textzone.innerHTML = `<span style="color: #93ff93; font-size: 16px">${team1.img.innerHTML} wins!</span>`
		else textzone.innerHTML = `<span style="color: #93ff93; font-size: 16px">${team2.img.innerHTML} wins!</span>`
